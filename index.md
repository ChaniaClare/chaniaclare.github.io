<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Chania Clare</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <style>
       
        body {
            font-family: "Arial", sans-serif;
            margin: 0;
            padding: 0;
            background-color: #f8f8f8;
            color: #333;
        }

        header {
            background-color: #333;
            color: #fff;
            padding: 2rem;
            text-align: center;
        }

        header h1 {
            font-size: 2.5rem;
            margin-bottom: 0;
        }

        header p {
            font-size: 1.2rem;
            margin-top: 0.5rem;
        }

        nav {
            background-color: #444;
            display: flex;
            justify-content: center;
            padding: 0.5rem;
        }

        nav a {
            color: #fff;
            text-decoration: none;
            padding: 0.5rem 1rem;
            transition: background-color 0.3s ease;
        }

        nav a:hover {
            background-color: #555;
        }

        main {
            padding: 1rem;
            max-width: 800px;
            margin: 0 auto;
        }

        section {
            margin-bottom: 2rem;
        }

        section h2 {
            font-size: 2rem;
            border-bottom: 1px solid #ccc;
            padding-bottom: 0.5rem;
            margin-bottom: 1rem;
        }

        section p {
            font-size: 1.1rem;
            line-height: 1.6;
        }

        .profile-pic {
            display: block;
            width: 150px;
            height: 150px;
            border-radius: 50%;
            margin: 0 auto;
            object-fit: cover;
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
        }

        footer {
            background-color: #333;
            color: #fff;
            text-align: center;
            padding: 1rem;
        }

        footer p {
            font-size: 1rem;
        }
    </style>
</head>
<body>
    <header>
        <img class="profile-pic" src="Chania Clare - photo.jpg" alt="Chania Clare">
        <h1>Chania Clare</h1>
        <p>PhD Student | Computational Systems and Synthetic Biology</p>
        <!-- Social Media Links -->
        <div class="social-links">
            <a href="https://www.linkedin.com/in/chania-clare/" target="_blank"><i class="fab fa-linkedin"></i></a>
            <a href="https://github.com/ChaniaClare" target="_blank"><i class="fab fa-github"></i></a>
            <a href="https://twitter.com/your-twitter-username" target="_blank"><i class="fab fa-twitter"></i></a>
        </div>
        <!-- CV Link -->
        <div class="cv-link">
            <a href="your-cv.pdf" download><i class="fas fa-file-pdf"></i> Download CV</a>
        </div>
    </header>
    
    <nav>
        <a href="#home">Home</a>
        <a href="#about">About</a>
        <a href="#projects">Projects</a>
        <a href="#contact">Contact</a>
    </nav>

    <main>
        <section id="home">
            <h2>Home</h2>
            <p>Write a brief introduction about yourself here.</p>
        </section>

        <section id="about">
            <h2>About Me</h2>
            <p>Write more details about yourself, your interests, and your academic journey.</p>
        </section>

        <section id="projects">
            <h2>Projects</h2>
            <p>List some of your notable projects with brief descriptions and links to their GitHub repositories or live demos.</p>
        </section>

        <section id="contact">
            <h2>Contact</h2>
            <p>Provide contact information such as email or social media profiles.</p>
        </section>
    </main>

    <footer>
        <p>&copy; 2023 Chania Clare. All rights reserved.</p>
    </footer>
</body>
</html>
